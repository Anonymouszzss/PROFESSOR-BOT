    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            ...
            btn = [[InlineKeyboardButton(text=f"⚡️{file.file_name}", callback_data=f'pmfile#{file.file_id}'),
                    InlineKeyboardButton(text=f"⚡️{get_size(file.file_size)}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            ...
            btn = [[InlineKeyboardButton(text=f"⚡️{file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}'),
                    InlineKeyboardButton(text=f"⚡️{get_size(file.file_size)}", callback_data=f'{pre}#{file.file_id}')] for file in files ]    
    if offset != "":
        key = f"{message.id}"
